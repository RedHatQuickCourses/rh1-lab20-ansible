...
. The first task in the playbook uses this hostname variable to add it to the `rh1` group using the `ansible.builtin.add_host` module.  

. This creates a temporary inventory, which is used by the next task in the playbook.  

. The next task installs a package on the hosts in the `rh1` group.  
