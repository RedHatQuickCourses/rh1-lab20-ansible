...
. First, you will need to disable the eda-debug rulebook under under Automation Decisions > Rulebook Activations. Next, after creating a new "project-http" inventory and adding localhost as a new host under the Hosts tab, remember to put localhost into disabled mode for this new inventory.
...
. Mark the Extra variable checkbox as Prompt on launch. doesn't make sense without more context.

image::lab005-019.jpg[lab005-019,100%,100%,link=self,window=_blank]

. Before stopping Apache service. Change *remote_user: root* to *remote_user: ec2-user* 
